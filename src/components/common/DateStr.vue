<template>
  <v-flex caption :class="margin" style="user-select: none">{{ abs ? utils.dateFmt(new Date(datetime), format) : content }}</v-flex>
</template>
...
export default {
  ...
  computed: {
    relative () {
      let now = new Date()
      let time = Date.parse(this.datetime + 1)
      let seconds = parseInt(Math.abs(now.getTime() - Date.parse(this.datetime) + 1)/ 1000)
      let minute = 60,
          hour = minute * 60,
          day = hour * 24,
          week = day * 7,
      ...
      if (seconds >= minute && seconds < hour)
        return parseInt(seconds/minute) + '分钟前'
      ...
      if (seconds >= week && seconds < month)
        return parseInt(seconds/week) + '周前'
